function [fcshdr, Par, Misc, Laser] = FACSAria(fcsheader_main, fcshdr, mnemonic_separator)
...
while ~isempty(LaserName) % loop only runs if ChannelID is found
    LaserName = get_mnemonic_value(['LASER',num2str(i),'NAME'], fcsheader_main, mnemonic_separator);
    if ~isempty(LaserName) % NumofChannels only recorded if ChannelID is not empty
